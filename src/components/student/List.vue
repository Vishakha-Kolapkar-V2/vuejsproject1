...
function FilteredList()  { 
  ...
  var data = JSON.parse(JSON.stringify(studentData.value));
  // console.log(JSON.parse(JSON.stringify(studentData.value)));
  
  var arr = data.filter((item) => {
    // console.log(item);
    // console.log(search.value);
        return item.fname.toLowerCase().includes(search.value.toLowerCase());
...
<template>
  ...
  <table class="table-auto w-full" v-else-if="studentData">
    ...
    <tbody class="text-center">
      <tr v-for="({ id, fname, lname, email, nmls }, i) in FilteredList()" :key="id">
        <td class="py-2">{{ ++i }}</td>
